...
library Utils {
    ...
    function calculateBNBReward(
        ...
        currentBalance = currentBalance.div(2); // Remember we have the BNB Reward pool and also the buyback function here
...
contract AdMonkey is Context, IERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xDA71E25bCD09eD576d803666D1fdBf1a8277B356); // Marketing Address
    ...
    uint256 private minimumTokensBeforeSwap = 100 * 10**4 * 10**9; 
    ...
    constructor (address payable routerAddress) public payable {
        
        BNBRewardsFee = bnbRewardFee;
        totalFee = bnbRewardFee.add(_liquidityFee).add(marketingDivisor).add(_taxFee);
    ...
    function canNextTransfer(address account) public view returns (uint256) {
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && from != uniswapV2Pair) {
            ...
            if (overMinimumTokenBalance) {
                ...
                //uint256 tokensToSwapLp = contractTokenBalance.mul(_liquidityFee).div(totalFee);
                //swapAndLiquify(tokensToSwapLp);
    
                uint256 sellTokens = balanceOf(address(this));
                swapAndSendTokens(sellTokens);
    ...
    function swapAndSendTokens(uint256 tokens) private {
        ...
        uint256 soldTokens = newBalance;
        
        uint256 piece = soldTokens.div(7);
        
        address payable contractAddress = payable(address(this));
        
        // Send to Marketing address
        transferToAddressETH(marketingAddress, piece.mul(marketingDivisor));
        // Send to Contract address
        transferToAddressETH(contractAddress, piece.mul(bnbRewardFee));
    ...
    function setBnbRewardFee(uint256 rewardFee) public onlyOwner {
